...
ADMIN_CODE = "admin123"  # código secreto para registrar admin
...
def registrar_usuario_publico():
    ...
    if role == 'admin':
        codigo = simpledialog.askstring("Registrar usuario", "Código secreto para registrar admin:", show='*')
        if codigo != ADMIN_CODE:
            messagebox.showerror("Código incorrecto", "Código secreto incorrecto. No se puede registrar como admin.")
            return None
